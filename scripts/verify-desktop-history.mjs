 ...
 *
 * 用法：node scripts/verify-desktop-history.mjs
 */
...
const SHOT_DIR = path.join(DESKTOP, 'probe-output', 'history-shots')
...
function main() {
	...
	const child = spawn(binary, ['.', '--history-probe'], {
		cwd: DESKTOP,
		env: {
			...process.env,
	...
	return exitCode.then((code) => {
		...
		const reportPath = path.join(DESKTOP, 'probe-output', 'history-report.json')
