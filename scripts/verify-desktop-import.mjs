 ...
 *
 * 用法：node scripts/verify-desktop-import.mjs
 */
...
const SHOT_DIR = path.join(DESKTOP, 'probe-output', 'import-shots')

const DATA_DIR = path.join(os.tmpdir(), `bbplayer-import-${Date.now()}`)
...
function main() {
	...
	console.log('=== Phase 3.3 验收：外部歌单导入 ===\n')
	console.log(`数据目录: ${DATA_DIR}`)
	console.log(`截图目录: ${SHOT_DIR}\n`)

	const child = spawn(binary, ['.', '--import-probe'], {
		cwd: DESKTOP,
		env: {
			...process.env,
	...
	return exitCode.then((code) => {
		...
		const reportPath = path.join(DESKTOP, 'probe-output', 'import-report.json')
