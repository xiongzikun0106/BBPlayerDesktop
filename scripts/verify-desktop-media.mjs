...
/**
 * Phase 4 验收：系统媒体集成（MediaSession / 任务栏缩略图 / 媒体键兜底）。
 *
...
const SHOT_DIR = path.join(DESKTOP, 'probe-output', 'media-shots')
...
function main() {
	...
	console.log('=== Phase 4 验收：系统媒体集成 ===\n')
	console.log(`数据目录: ${DATA_DIR}`)
	console.log(`截图目录: ${SHOT_DIR}`)
	console.log(`平台: ${process.platform}\n`)

	const child = spawn(binary, ['.', '--media-probe'], {
		cwd: DESKTOP,
		env: {
			...process.env,
	...
	return exitCode.then((code) => {
		...
		const reportPath = path.join(DESKTOP, 'probe-output', 'media-report.json')
