...
- `pnpm verify:desktop:ui` —— 131 条断言（设计系统级：字阶、选中态一致性、
...
- ⚠️ **验证盲区（已知，未解决）**：根 `oxlint.config.mts` 把所有 `.js` 文件
  排除了（移动端有大量生成的 JS），于是桌面端渲染进程（`renderer` 下的 `.js`）
  **从来没有被 lint 过** —— 探针是 `.cjs`，会被查；渲染进程不会，
  用 `--no-ignore` 也绕不过。实测把 `share.js` / `player.js` 复制成 `.cjs`
  再跑，有 6 条**风格类**问题（`no-console`、函数作用域、变量遮蔽、
  多余的展开兜底），没有正确性问题。
  要不要给渲染进程单独开一份 lint 配置是个**待定决策**（会影响移动端）。
