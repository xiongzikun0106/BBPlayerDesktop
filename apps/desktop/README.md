...
- **控件的外观必须与它是否可交互一致**：禁用的滑杆如果还画着主色轨道，
  ...
  DOM 正常、尺寸正常，**只有人眼看得出**。
- **正在播放页是三栏**（阶段 D，按用户给的网易云参考图的**布局**，视觉仍是本项目令牌）：
  左大封面 + 曲名/歌手、中**歌词**、右**播放列表（默认收起）**，每栏各自滚动。
  ⚠️ 歌词与队列都是**同一份 DOM 搬进去**的（`placeLyrics` / `placeQueue`），
  绝不各渲染一份 —— 两份会让 testid 重复、拖拽落到错误的树上，而界面看着正常。
  ⚠️ `placeLyrics` 搬的是整个 `.panel[data-panel="lyrics"]` **壳**（里面有
  「重新匹配 / 独立窗口 / 状态」工具条），不是内层 `#lyrics-panel` 挂载点；
  而且那个壳**没有 `id`**（只有 `data-panel`）—— 按 id 找会静默拿到 null，
  于是中栏永远空着，**而所有断言照过**（第一版就是这样，靠新断言 + 截图才发现）。
- **「呼出 / 收起播放列表」三处共用一个实现**（`toggleQueueColumn()`）：播放条上的
  按钮（按参考图放在「下一首」右边）、`Ctrl+Q`、顶部菜单那一项。不在播放页就先
  进去并把列表栏显示出来，已经在播放页就切换显隐。
- **队列行 = 序号 + 曲绘封面 + 标题/歌手 + 时长**（与其它列表行同一套约定）。
- **左栏宽度可拖拽**（`renderer/splitter.js`）：`.app` 第一列走 `--sidebar-width`，
  于是"左栏收起"与"右栏收起"**互不覆盖**（两个状态都去写整条
  `grid-template-columns` 的话，后写的会吃掉前一个，表现为"某次展开/收起失灵"）。
  宽度存进设置、双击还原、拖到极窄自动收起（**0 是合法的已收起值**，校正时不能
  把它 clamp 成最小值）。
  ⚠️ `settings.get()` 返回的是 `describe()`，真正的值在 `data.settings.*` ——
  少剥一层就是 `undefined`，"记住宽度"会**静默失效**（这个形状仓库栽过两次）。
  ⚠️ `waitFor()` **不会 await 异步表达式**（它先 `JSON.stringify`），要等异步 IPC
  的结果必须手写"睡一会儿 + 显式 await"。
- **冻结表头**（`.track-table th`）有四条不变量，缺一条就会出"发白的横带 /
  方角压住卡片圆角 / 表头和表体对不上 / 缝里露出半行"这类问题：
  1. **底色用它所在容器的面**（`var(--surface)`），不是页面底色 `--bg`；
  2. `top: 0`（不是负值 —— 负值会让表头钻出滚动视口、方角盖住卡片圆角）；
  3. `z-index` ≥ 1，否则行会跟它抢绘制顺序；
  4. 表头**上方那条缝**要用 `th::before` 补同色块盖住：粘性元素的活动范围被
     它的包含块（`<table>`）限制住，而表格上边缘在滚动容器里缩了 `padding-top`，
     所以表头最多只能粘到那里，上面露出一条缝、滚过去的行会从缝里露出来。
     缝高 = 所在容器的上内边距（`--sticky-head-gap`），换容器要跟着设。
     ⚠️ **"表体单独设了对齐/宽度"的列，表头必须跟着设** —— 序号/时长列原来只有
     `td` 是右对齐，表头落到通用规则的左对齐，同一列两种对齐，滚动时看起来就是
     "表头和内容对不上"（用户圈的就是它，不是滚动条的问题）。
- **表头/表体的"有没有被盖住"要能机械化验证**：断言用 `elementFromPoint`
  去点表头带里的几个位置，命中的必须是 `TH` 自己 —— 颜色、位置、对齐全对，
  仍然可能有行从表头那里画出来，而**坐标断言全是绿的**。
- **主页 = 听歌频率热力图 + 快捷入口 + 最近更新（歌单）+ 播放历史**（阶段 6d）。
