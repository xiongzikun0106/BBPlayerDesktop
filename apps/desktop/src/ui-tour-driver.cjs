...
async function shot(window, name, note) {
	...
	report.shots.push({ name, note, file, ...size })
	console.log(`  📷 ${name}  ${size.width}x${size.height}  ${note}`)
	return file
}
...
async function run(window) {
	...
	console.log('\n=== 7) 设置抽屉的每个页签 ===')
	await click(window, '[data-testid="settings-open"]')
	await sleep(800)
	for (const [tab, name, note] of [
		['appearance', '14-settings-appearance', '设置 › 外观'],
		['playback', '15-settings-playback', '设置 › 播放'],
		['download', '16-settings-download', '设置 › 下载'],
		['backup', '17-settings-backup', '设置 › 备份与恢复'],
	]) {
		await click(window, `[data-settings-tab="${tab}"]`)
		await sleep(900)
		await shot(window, name, note)
	}
	// 诊断信息展开看一眼
	...
	await click(window, '[data-testid="settings-close"]')
	await sleep(600)

	console.log('\n=== 8) 登录弹窗的每个页签 ===')
	await click(window, '[data-testid="account-open"]')
	await sleep(900)
	await shot(window, '19-login-qr', '登录弹窗 › 扫码')
	for (const [tab, name, note] of [
		['cookie', '20-login-cookie', '登录弹窗 › Cookie'],
		['password', '21-login-password', '登录弹窗 › 密码'],
		['account', '22-login-account', '登录弹窗 › 账号'],
	]) {
	...
	await sleep(1500)
	await shot(window, '23-share', '共享歌单面板')
	...
	await click(window, '[data-testid="search-button"]')
	await sleep(4500)
	await shot(window, '24-search-empty-result', '搜索无结果（空状态）')
	...
	await click(window, '[data-testid="nav-library"]')
	await sleep(1200)
	await shot(window, '25-narrow-library', '窄窗口（1040x800）')
